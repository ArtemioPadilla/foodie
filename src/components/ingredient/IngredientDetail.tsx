...
export const IngredientDetail: React.FC<IngredientDetailProps> = ({
  ...
  const preparationInstructions = ingredient.preparationInstructions 
    ? getTranslated(ingredient.preparationInstructions) 
    : null;

  // Build dietary tags to display
  const dietaryTags: { key: string; label: string; color: string }[] = [];
  
  if (ingredient.tags.vegan) {
    dietaryTags.push({ key: 'vegan', label: t('dietary.vegan'), color: 'success' });
  }
  if (ingredient.tags.vegetarian) {
  ...
  return (
    <div className={cn('space-y-6', className)}>
        ...
        {dietaryTags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            {dietaryTags.map((tag) => (
              <Badge 
                key={tag.key} 
                variant={tag.color as 'success' | 'warning' | 'info' | 'default'} 
                size="md"
              >
                {tag.label}
              </Badge>
      ...
      {ingredient.isComposite && ingredient.components && ingredient.components.length > 0 && (
        <Card padding="lg">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            {t('ingredient.compositeDescription', 'This ingredient is made from the following components:')}
          </p>
              ...
              return (
                ...
                >
                  ...
                  <Link 
                    ...
                    aria-label={t('ingredient.viewIngredient', 'View ingredient')}
                  >
          ...
          {ingredient.yield && (
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                <span className="font-medium">{t('ingredient.yield', 'Yield')}:</span>{' '}
      ...
      {/* Preparation Instructions for Composite Ingredients */}
      {ingredient.isComposite && preparationInstructions && (
        <Card padding="lg">
          <div className="flex items-center gap-2 mb-4">
            <ChefHat className="h-5 w-5 text-emerald-600 dark:text-emerald-400" />
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
              {t('ingredient.preparationInstructions', 'Preparation Instructions')}
            </h2>
          </div>
      ...
      <Card padding="lg">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
            {t('ingredient.storageInstructions', 'Storage Instructions')}
          </h2>
        </div>
